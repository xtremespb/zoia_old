...
module.exports = function(app, router) {
    ...
    const process = async(req, res) => {
        ...
        if (!code || !code.match(/^[0-9a-f]{1,128}$/)) {
        ...
        try {
            ...
            if (!data1 || !data1.email || !data.user_id) {
            ...
            const passwordHash = crypto.createHash('md5').update(config.salt + Date.now()).digest('hex');
            /*let updResult = await db.collection('users').update({email: email}, {
                username: `vk${userID}`,
                email: email,
            ...
            renderPage(req, res, response1 + ' ' + response2);
        } catch (e) {
            log.error(e);            
            return renderPage(req, res, i18n.get().__(locale, 'Error while processing your authorization.') + `<br>${e}`);
        }
    };
