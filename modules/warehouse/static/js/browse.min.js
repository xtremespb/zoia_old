...
"use strict";var currentDir="",historyDir=[],spinnerDialog=void 0,fileDialog=void 0,uploadDialog=void 0,fileDialogAction=void 0,fileDialogValue=void 0,clipboardDir=void 0,clipboardData=void 0,clipboardOperation=void 0,uploader=void 0,uploadFailed=void 0,templates={d:'<div class="zoia-browse-item zoia-browse-item-d" data="{filename}"><div class="zoia-browse-item-img za-flex za-flex-center za-flex-middle"><span za-icon="icon:folder;ratio:2"></span></div><div class="zoia-browse-item-text"{tooltip}>{filename}</div></div>',f:'<div class="zoia-browse-item zoia-browse-item-f" data="{filename}"><div class="zoia-browse-item-img za-flex za-flex-center za-flex-middle">{innerContent}</div><div class="zoia-browse-item-text"{tooltip}>{filename}</div></div>',o:""},template=function(e,a){for(var i in a)e=e.replace(new RegExp("{"+i+"}","g"),a[i]);return e},setButtonsState=function(){$(".zoia-browse-ctl-button").attr("disabled",!0),$(".zoia-browse-ctl-refresh, .zoia-browse-ctl-upload, .zoia-browse-ctl-newdir").attr("disabled",!1);var e=$("#zoia-browse-content").getSelected("zoia-browse-item-selected");e.length&&(1===e.length&&($(".zoia-browse-ctl-rename").attr("disabled",!1),$('.zoia-browse-item[data="'+e[0]+'"]').hasClass("zoia-browse-item-f")&&$("#zoiaBrowseSelectBtn").attr("disabled",!1)),$(".zoia-browse-ctl-delete, .zoia-browse-ctl-cut, .zoia-browse-ctl-copy").attr("disabled",!1)),clipboardOperation&&$(".zoia-browse-ctl-paste").attr("disabled",!1),currentDir&&$(".zoia-browse-ctl-up").attr("disabled",!1)},shiftySelectHandler=function(){setButtonsState()},shiftyUnselectHandler=function(){},load=function(){spinnerDialog.show(),$("#zoia-browse-content").html(""),$.ajax({type:"POST",url:"/api/warehouse/browse/list",data:{path:(historyDir.join("/")+"/"+currentDir).replace(/^\//,"")},cache:!1}).done(function(e){if(spinnerDialog.hide(),1===e.status){var a="";for(var i in e.files){var t=e.files[i],o="";t.filename.length>14&&(o=' title="'+t.filename+'" za-tooltip="delay:300"');var l="";if(t.ext&&!t.thumb)switch(t.ext){case"jpg":case"jpeg":case"gif":case"png":l='<span za-icon="icon:image;ratio:2">';break;default:l='<span za-icon="icon:file;ratio:2">'}t.thumb&&(l='<img src="/warehouse/static/storage/'+(historyDir.join("/")+"/"+currentDir).replace(/^\//,"")+"/___tn_"+t.filename+'" class="zoia-browse-item-thumb">'),a+=template(templates[t.type],{filename:t.filename,type:t.type,tooltip:o,innerContent:l})}e.files&&e.files.length||(a='<div style="padding-left:15px">'+lang["No files to display"]+"</div>"),$("#zoia-browse-content").html(a),$(".zoia-browse-item").dblclick(dblClickHandler),$(".zoia-browse-item").on("doubletap",dblClickHandler),$(".zoia-browse-item").shifty({className:"zoia-browse-item-selected",select:function(){shiftySelectHandler()},unselect:function(){shiftyUnselectHandler()}})}else switch(spinnerDialog.hide(),e.status){case-1:$zUI.notification(lang["Directory doesn't exists"],{status:"danger",timeout:1500});break;default:$zUI.notification(lang["Could not get directory content"],{status:"danger",timeout:1500})}}).fail(function(){$zUI.notification(lang["Could not get directory content"],{status:"danger",timeout:1500})})},dblClickHandler=function(){$(this).hasClass("zoia-browse-item-d")&&(currentDir&&historyDir.push(currentDir),currentDir=$(this).attr("data"),load(),setButtonsState())},btnUpHandler=function(e){e.preventDefault(),$(".zoia-browse-item").removeClass("zoia-browse-item-selected"),currentDir=0===historyDir.length?"":historyDir.pop(),load(),setButtonsState()},btnNewDirHandler=function(){fileDialog.show(),$(".zoiaFileDialogTitle").html(lang["New folder"]),$("#zoiaFileDialogInput").focus(),$("#zoiaFileDialogInput").val(""),fileDialogAction="folder"},btnRenameHandler=function(e){fileDialog.show(),$(".zoiaFileDialogTitle").html(lang.Rename),$("#zoiaFileDialogInput").focus(),$("#zoiaFileDialogInput").val(e),fileDialogAction="rename",fileDialogValue=e},btnCutCopyHandler=function(){var e=$("#zoia-browse-content").getSelected("zoia-browse-item-selected");return!(!e||0===e.length)&&($(".zoia-browse-ctl-clipboard").removeClass("za-button-secondary"),clipboardData=e,clipboardDir=(historyDir.join("/")+"/"+currentDir).replace(/^\//,""),!0)},btnPasteHandler=function(){clipboardDir!==(historyDir.join("/")+"/"+currentDir).replace(/^\//,"")?clipboardOperation&&(spinnerDialog.show(),$.ajax({type:"POST",url:"/api/warehouse/browse/paste",data:{pathSource:clipboardDir,pathDestination:(historyDir.join("/")+"/"+currentDir).replace(/^\//,""),files:clipboardData,operation:clipboardOperation},cache:!1}).done(function(e){$(".zoia-browse-ctl-clipboard").removeClass("za-button-secondary"),clipboardData=null,clipboardOperation=null,clipboardDir=null,1===e.status?($zUI.notification(lang["Operation was successful"],{status:"success",timeout:1500}),load()):(spinnerDialog.hide(),$zUI.notification(lang["Cannot paste one or more files"],{status:"danger",timeout:1500}))}).fail(function(){spinnerDialog.hide(),$(".zoia-browse-ctl-clipboard").removeClass("za-button-secondary"),clipboardData=null,clipboardOperation=null,clipboardDir=null,$zUI.notification(lang["Cannot paste one or more files"],{status:"danger",timeout:1500})})):$zUI.modal.alert(lang["Cannot paste files to the same directory"],{labels:{ok:lang.OK}})},btnDeleteHandler=function(){var e=$("#zoia-browse-content").getSelected("zoia-browse-item-selected");e&&0!==e.length&&$zUI.modal.confirm(lang["You are going to delete the following file(s)"]+":<br><br>"+e.join(", "),{labels:{ok:lang.OK,cancel:lang.Cancel}}).then(function(){$.ajax({type:"POST",url:"/api/warehouse/browse/delete",data:{path:(historyDir.join("/")+"/"+currentDir).replace(/^\//,""),files:e},cache:!1}).done(function(e){1===e.status?($zUI.notification(lang["Operation was successful"],{status:"success",timeout:1500}),load()):(spinnerDialog.hide(),$zUI.notification(lang["Error while deleting one or more files"],{status:"danger",timeout:1500}))}).fail(function(){spinnerDialog.hide(),$zUI.notification(lang["Error while deleting one or more files"],{status:"danger",timeout:1500})})})},formSubmitHandler=function(){var e=$("#zoiaFileDialogInput").val();if(!e||"string"!=typeof e||e.length>40||!e.match(/^[a-zA-Z0-9_\-\.;\s]+$/))$zUI.notification(lang["Invalid name, please use latin characters and numbers only"],{status:"danger",timeout:1500});else switch(fileDialogAction){case"folder":fileDialog.hide(),spinnerDialog.show(),$.ajax({type:"POST",url:"/api/warehouse/browse/folder/create",data:{path:(historyDir.join("/")+"/"+currentDir).replace(/^\//,""),name:e},cache:!1}).done(function(e){1===e.status?($zUI.notification(lang["Operation was successful"],{status:"success",timeout:1500}),load()):(spinnerDialog.hide(),$zUI.notification(lang["Error while creating new folder"],{status:"danger",timeout:1500}))}).fail(function(){spinnerDialog.hide(),$zUI.notification(lang["Error while creating new folder"],{status:"danger",timeout:1500})});break;case"rename":fileDialog.hide(),spinnerDialog.show(),$.ajax({type:"POST",url:"/api/warehouse/browse/rename",data:{path:(historyDir.join("/")+"/"+currentDir).replace(/^\//,""),nameOld:fileDialogValue,nameNew:e},cache:!1}).done(function(e){1===e.status?($zUI.notification(lang["Operation was successful"],{status:"success",timeout:1500}),load()):(spinnerDialog.hide(),$zUI.notification(lang["Could not rename selected file"],{status:"danger",timeout:1500}))}).fail(function(){spinnerDialog.hide(),$zUI.notification(lang["Could not rename selected file"],{status:"danger",timeout:1500})})}},uploadClearBtnHandler=function(){uploader.splice(0),$("#zoia-upload-files").html("").hide()},uploadStartHandler=function(){uploader.settings.multipart_params={dir:(historyDir.join("/")+"/"+currentDir).replace(/^\//,"")},uploadFailed=!1,uploader.start()},initUploader=function(){(uploader=new plupload.Uploader({browse_button:"zoia-upload-area",runtimes:"html5,html4",url:"/api/warehouse/browse/upload",drop_element:"zoia-upload-area",filters:{max_file_size:"100mb"}})).init(),uploader.bind("FilesAdded",function(e,a){var i="";plupload.each(a,function(e){i+='<div><div class="zoia-upload-files-label">'+e.name+"&nbsp;("+plupload.formatSize(e.size)+')</div><progress id="'+e.id+'" class="za-progress" value="0" max="100"></progress></div>'}),$("#zoia-upload-files").html(i),$("#zoia-upload-files").show()}),uploader.bind("Error",function(){$zUI.notification(lang["Cannot upload"]+": "+file.name,{status:"danger",timeout:1500})}),uploader.bind("UploadProgress",function(e,a){$("#"+a.id).attr("value",a.percent)}),uploader.bind("FileUploaded",function(e,a,i){try{1!==JSON.parse(i.response).status?(uploadFailed=!0,$zUI.notification(lang["Cannot upload"]+": "+a.name,{status:"danger",timeout:1500})):$("#"+a.id).parent().remove()}catch(e){}}),uploader.bind("UploadComplete",function(){uploadFailed||uploadDialog.hide(),load()})},getUrlParam=function(e){var a=new RegExp("(?:[?&]|&amp;)"+e+"=([^&]+)","i"),i=window.location.search.match(a);return i&&i.length>1?i[1]:""};$(document).ready(function(){spinnerDialog=$zUI.modal("#zoiaSpinnerDialog",{bgClose:!1,escClose:!1,stack:!0}),fileDialog=$zUI.modal("#zoiaFileDialog",{bgClose:!1,escClose:!1,stack:!0}),uploadDialog=$zUI.modal("#zoiaUploadDialog",{bgClose:!1,escClose:!1,stack:!0}),$("#zoia-browse-content").click(function(e){"zoia-browse-content"===e.target.id&&($(".zoia-browse-item").removeClass("zoia-browse-item-selected"),setButtonsState())}),$(".zoia-browse-ctl-up").click(btnUpHandler),$(".zoia-browse-ctl-newdir").click(btnNewDirHandler),$(".zoia-browse-ctl-delete").click(btnDeleteHandler),$(".zoia-browse-ctl-upload").click(function(){uploadFailed=!1,uploader.splice(0),$("#zoia-upload-files").html("").hide(),uploadDialog.show()}),$(".zoia-browse-ctl-refresh").click(load),$(".zoia-browse-ctl-copy").click(function(){btnCutCopyHandler()&&(clipboardOperation="copy",setButtonsState(),$(".zoia-browse-ctl-copy").addClass("za-button-secondary"))}),$(".zoia-browse-ctl-cut").click(function(){btnCutCopyHandler()&&(clipboardOperation="cut",setButtonsState(),$(".zoia-browse-ctl-cut").addClass("za-button-secondary"))}),$(".zoia-browse-ctl-paste").click(btnPasteHandler),$(".zoia-browse-ctl-rename").click(function(){var e=$("#zoia-browse-content").getSelected("zoia-browse-item-selected");!e.length||e.length>1||btnRenameHandler(e[0])}),$("#zoiaFileDialogForm").submit(function(e){e.preventDefault(),formSubmitHandler()}),$("#btnUploadClear").click(uploadClearBtnHandler),$("#btnUpload").click(uploadStartHandler),$("#zoiaBrowseCancelBtn").click(function(){window.close()}),$("#zoiaBrowseSelectBtn").click(function(){var e=$("#zoia-browse-content").getSelected("zoia-browse-item-selected");if(e.length&&1===e.length&&$('.zoia-browse-item[data="'+e[0]+'"]').hasClass("zoia-browse-item-f")){var a=(historyDir.join("/")+"/"+currentDir+"/"+e[0]).replace(/\/\//gm,"");"/"!==a.charAt(0)&&(a="/"+a),a="/warehouse/static/storage"+a;var i=getUrlParam("CKEditorFuncNum");window.opener.CKEDITOR.tools.callFunction(i,a),window.close()}}),initUploader(),load(),setButtonsState()});