    ...
    $.ajax({
        ...
        if (res.status === 1) {
            ...
            for (let i in res.files) {
                ...
                if (item.thumb) {
                    innerContent = '<img src="/warehouse/static/storage/' + (historyDir.join('/') + '/' + currentDir).replace(/^\//, '') + '/___tn_' + item.filename + '" class="zoia-browse-item-thumb">';
                }
...
const btnPasteHandler = () => {
    ...
    $.ajax({
        type: 'POST',
        url: '/api/warehouse/browse/paste',
        data: {
...
const btnDeleteHandler = () => {
    ...
    $zUI.modal.confirm(lang['You are going to delete the following file(s)'] + ':<br><br>' + selection.join(', '), { labels: { ok: lang['OK'], cancel: lang['Cancel'] } }).then(function() {
        $.ajax({
            type: 'POST',
            url: '/api/warehouse/browse/delete',
            data: {
                path: (historyDir.join('/') + '/' + currentDir).replace(/^\//, ''),
                files: selection
            },
...
const formSubmitHandler = () => {
    ...
    switch (fileDialogAction) {
        case 'folder':
            ...
            $.ajax({
                type: 'POST',
                url: '/api/warehouse/browse/folder/create',
                data: {
                    path: (historyDir.join('/') + '/' + currentDir).replace(/^\//, ''),
                    name: name
                },
        ...
        case 'rename':
            ...
            $.ajax({
                type: 'POST',
                url: '/api/warehouse/browse/rename',
                data: {
                    path: (historyDir.join('/') + '/' + currentDir).replace(/^\//, ''),
...
const initUploader = () => {
    uploader = new plupload.Uploader({
        ...
        url: '/api/warehouse/browse/upload',
...
$(document).ready(() => {
    ...
    $('#zoiaBrowseSelectBtn').click(() => {
        ...
        if (selection.length && selection.length === 1 && $('.zoia-browse-item[data="' + selection[0] + '"]').hasClass('zoia-browse-item-f')) {
            ...
            url = '/warehouse/static/storage' + url;
